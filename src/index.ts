...
import { z } from "zod";
import { createPeonTools, type CrossProjectSearchToolResult, type SearchMemoryToolResult } from "./tools.js";
import type { BrainInspection, ProjectContext } from "./types.js";
...
const projectName = (path: string): string => path.split("/").filter(Boolean).pop() ?? "project";
 ...
 */
function compactContext(ctx: ProjectContext): string {
...
function compactSearch(result: SearchMemoryToolResult): string {
  return result.injectionPreview?.trim() || `No memory matched "${result.query}".`;
}

function compactBrain(result: BrainInspection): string {
  const counts: Record<string, number> = {};
  for (const record of result.records) counts[record.status] = (counts[record.status] ?? 0) + 1;
...
function compactCrossProject(result: CrossProjectSearchToolResult): string {
...
server.registerTool(
  ...
  async (input) => ({
    content: [
      {
        type: "text",
        text: compactCrossProject(
          await tools.crossProjectSearch({
            query: input.query,
