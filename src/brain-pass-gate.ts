...
const lastPassByFile = new Map<string, { mtimeMs: number; size: number; at: number }>();

/** Test hook: how long an unchanged brain may go without a pass. */
export function setBrainPassIdleWindow(ms: number): void {
  idleWindowMs = ms;
}

/** Test hook: forget completed passes and restore the default window. */
export function resetBrainPassGate(): void {
...
export async function isIdleSinceLastPass(file: string): Promise<boolean> {
  const previous = lastPassByFile.get(file);
  if (!previous || Date.now() - previous.at >= idleWindowMs) return false;
...
export async function recordCompletedPass(file: string): Promise<void> {
  const current = await stat(file).catch(() => undefined);
  if (current) lastPassByFile.set(file, { mtimeMs: current.mtimeMs, size: current.size, at: Date.now() });
}
