...
import { contentHash, type EmbeddingClient, type EmbeddingVector } from "./embeddings.js";
...
export class EmbeddingStore {
  ...
  private cache?: { mtimeMs: number; map: Map<string, StoredEmbedding> };

  private constructor(private readonly filePath: string) {}

  static async open(memoryDir: string): Promise<EmbeddingStore> {
  ...
  async load(): Promise<Map<string, StoredEmbedding>> {
    let mtimeMs = 0;
    try {
    ...
    for (const line of raw.split(/\r?\n/)) {
      ...
      try {
        ...
        if (stored) map.set(stored.id, stored);
      } catch {
   ...
   */
  async sync(records: MemoryRecord[], client: EmbeddingClient | null): Promise<SyncResult> {
    if (!client) {
    ...
    for (const [id, stored] of result) vectorById.set(id, stored.vector);
  ...
  async vectorById(): Promise<Map<string, EmbeddingVector>> {
    ...
    for (const [id, value] of stored) map.set(id, value.vector);
    return map;
  }

  private async persist(records: MemoryRecord[], result: Map<string, StoredEmbedding>): Promise<void> {
    // Write in record order for stable diffs; only persist vectors we actually have.
    const lines = records
      .map((record) => result.get(record.id))
      .filter((value): value is StoredEmbedding => Boolean(value))
...
export function encodeVector(vector: EmbeddingVector): string {
 ...
 */
function parseStoredLine(value: unknown): StoredEmbedding | null {
