...
import { cosineSimilarity, createEmbeddingClient, type EmbeddingClient, type EmbeddingVector, embedQueryWithin, DEFAULT_QUERY_EMBED_TIMEOUT_MS } from "./embeddings.js";
...
import { currentAsOf, changesBetween, type ChangeEntry } from "./temporal.js";
...
import type { BrainInspection, MemoryGraph, MemoryRecord, MemoryRecordInput, MemoryType, PeonEvent, PeonRole, PeonSession, ProcessedMemory, ProcessingState, ProjectContext } from "./types.js";
...
function withProjectWriteLock<T>(key: string, fn: () => Promise<T>): Promise<T> {
...
function projectionsFor(dim: number): Float32Array[] {
  ...
  for (let p = 0; p < DEDUP_BANDS * DEDUP_BITS_PER_BAND; p += 1) {
    ...
    for (let d = 0; d < dim; d += 1) plane[d] = rand() * 2 - 1;
    planes.push(plane);
  }
 ...
 */
function bandKeys(vector: EmbeddingVector): string[] {
  ...
  for (let band = 0; band < DEDUP_BANDS; band += 1) {
    ...
    for (let b = 0; b < DEDUP_BITS_PER_BAND; b += 1) {
      ...
      let dot = 0;
      for (let d = 0; d < dim; d += 1) dot += vector[d] * plane[d];
      bits += dot >= 0 ? "1" : "0";
    }
    keys.push(`${band}:${bits}`);
  }
...
export class PeonMemoryStore {
  private readonly sessions = new Map<string, PeonSession>();
  ...
  private constructor(
    ...
    private readonly queryEmbedTimeoutMs: number = DEFAULT_QUERY_EMBED_TIMEOUT_MS
  ) {}

  static async open(options: OpenMemoryStoreOptions): Promise<PeonMemoryStore> {
    ...
    const embeddingClient =
      options.embeddingClient !== undefined ? options.embeddingClient : createEmbeddingClient({ config });
    const store = new PeonMemoryStore(
      options.projectPath,
      memoryDir,
      embeddingClient,
      config.queryEmbedTimeoutMs ?? DEFAULT_QUERY_EMBED_TIMEOUT_MS
    );
  ...
  async startSession(input: StartSessionInput): Promise<PeonSession> {
    const session: PeonSession = {
  ...
  async recordMessage(input: RecordMessageInput): Promise<PeonEvent> {
  ...
  async recordEvent(input: RecordEventInput): Promise<PeonEvent> {
  ...
  async endSession(input: EndSessionInput): Promise<PeonSession> {
    const session = this.requireSession(input.sessionId);
    const ended: PeonSession = { ...session, endedAt: new Date().toISOString() };
  ...
  async getContext(input: GetContextInput = {}): Promise<ProjectContext> {
    ...
    const ofType = (...types: MemoryType[]) => active.filter((item) => types.includes(item.record.type));
    const section = async (records: RankedMemoryRecord[], file: string, title: string, frac: number): Promise<string> => {
      const budget = Math.floor(maxChars * frac);
      if (records.length > 0) return formatContextRecords(records, budget, title);
    ...
    const safeSections = Object.fromEntries(Object.entries(sections).map(([k, v]) => [k, redactSecrets(v)])) as typeof sections;
    ...
    const genuinelyMatches = (item: RankedMemoryRecord): boolean =>
      item.reasons.some(
        (r) =>
          r.kind === "query_term" ||
          r.kind === "file" ||
  ...
  async inspectBrain(input: GetContextInput = {}): Promise<BrainInspection> {
  ...
  async replaceMemoryRecords(input: MemoryRecord[]): Promise<void> {
    ...
    await atomicWrite(
      join(this.memoryDir, "brain", "memories.jsonl"),
  ...
  async updateMemoryRecord(id: string, patch: MemoryPatch): Promise<MemoryRecord | null> {
  ...
  async deleteMemoryRecord(id: string): Promise<boolean> {
    return this.withWriteLock(async () => {
      const records = await this.readMemoryRecords();
  ...
  async setMemoryRecordPinned(id: string, pinned: boolean): Promise<MemoryRecord | null> {
    return this.withWriteLock(async () => {
      const records = await this.readMemoryRecords();
      if (!records.some((record) => record.id === id)) return null;
  ...
  async mergeMemoryRecords(keepId: string, dropId: string): Promise<MemoryRecord | null> {
    return this.withWriteLock(async () => {
      const records = await this.readMemoryRecords();
      const next = applyMerge(records, keepId, dropId, new Date().toISOString());
      if (next.length === records.length && keepId !== dropId) return null;
      await this.replaceMemoryRecords(next);
   ...
   */
  async runBrainPass(options: { recalledIds?: string[]; summarize?: Summarizer; minClusterSize?: number } = {}): Promise<BrainAction[]> {
    ...
    if (!options.recalledIds?.length && !options.summarize && (await isIdleSinceLastPass(memoriesFile))) return [];
    return this.withWriteLock(async () => {
      const records = await this.readMemoryRecords();
      if (records.length === 0) {
      ...
      const { records: curated, actions } = await runSleepCycle(records, {
        ...
        makeSummaryId: (entity, content) =>
          `mem_summary_${stableMemoryId("summary", entity).slice(-12)}_${stableMemoryId("summary", content).slice(-8)}`
      });
      ...
      if (actions.length > 0) await this.appendJsonl("brain/brain-actions.jsonl", { at: now, actions });
      await recordCompletedPass(memoriesFile);
      return actions;
    });
  }
  ...
  async archiveRecords(ids: readonly string[], reason: string): Promise<number> {
    ...
    return this.withWriteLock(async () => {
      ...
      const next = records.map((record) => {
        if (idSet.has(record.id) && record.status === "active" && !record.pinned) {
          archived += 1;
          return { ...record, status: "archived" as const, updatedAt: now, source: { ...record.source, reason } };
        }
        return record;
      });
      if (archived > 0) await this.replaceMemoryRecords(next);
      return archived;
    });
  }

  /** Recent autonomous actions the brain took — powers the cockpit "what the brain did" feed. */
  async readBrainActions(limit = 50): Promise<Array<{ at: string; actions: BrainAction[] }>> {
    const rows = await this.readJsonl("brain/brain-actions.jsonl");
    return rows.slice(-limit).reverse() as Array<{ at: string; actions: BrainAction[] }>;
  }

  private async snapshotBackup(records: MemoryRecord[], now: string): Promise<void> {
  ...
  async currentAsOf(at: string | number | Date): Promise<MemoryRecord[]> {
    return currentAsOf(await this.readMemoryRecords(), at);
  }

  /** Time-travel: the changelog (added / superseded / retired) over [from, to]. */
  async changesBetween(from: string | number | Date, to: string | number | Date): Promise<ChangeEntry[]> {
   ...
   */
  async rankEpisodes(query: string | undefined, options: { limit?: number } = {}): Promise<RankedMemoryRecord[]> {
    ...
    const substantive = [...messages, ...events].filter(
      (e) => typeof e.content === "string" && (e.content as string).trim().length > 0 && !TOOL_TYPES.has(String(e.type))
    );
    ...
    const episodes: MemoryRecord[] = candidates
      .map((e) => {
  ...
  async rankRecords(
    ...
    options: { limit?: number; expandGraph?: boolean } = {}
  ...
  private async buildSemanticInput(
  ...
  ): Promise<SemanticRetrievalInput | undefined> {
    ...
    try {
      ...
      const vectorById = await this.embeddingStore.vectorById();
      if (vectorById.size === 0) return undefined;
  ...
  async writeQualityReport(report: MemoryQualityReport): Promise<void> {
    await writeFile(join(this.memoryDir, "brain", "quality-report.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
  }

  async readRawMemory(maxChars = 12000): Promise<string> {
    const [messages, events] = await Promise.all([
      this.readJsonl("raw/messages.jsonl"),
      this.readJsonl("raw/events.jsonl")
    ]);
   ...
   */
  async readRawMemoryDelta(
    afterEventId?: string,
    maxChars?: number
  ): Promise<{ text: string; lastEventId?: string; capped: boolean }> {
    ...
    const sorted = [...messages, ...events].sort((left, right) =>
    ...
    for (const entry of slice) {
      ...
      lines.push(line);
      total += line.length + 1;
      boundaryId = (entry.id as string | undefined) ?? boundaryId;
    }

    return { text: lines.join("\n"), lastEventId: boundaryId, capped };
  }

  async applyProcessedMemory(
    memory: ProcessedMemory,
    source: { reason?: string } = {},
    modelEntities?: Map<string, string[]>
  ): Promise<ApplyMemoryStats> {
    const summary = [
      "# Project Summary",
      "",
   ...
   */
  async mergeSimilarActiveRecords(
    records: MemoryRecord[],
    threshold = 0.9,
    options: { maxActive?: number; exhaustive?: boolean } = {}
  ): Promise<{ records: MemoryRecord[]; merged: number; comparisons: number }> {
    if (!this.embeddingClient || !this.embeddingStore) return { records, merged: 0, comparisons: 0 };
    ...
    if (vectorById.size === 0) return { records, merged: 0, comparisons: 0 };

    const active = records.filter((record) => record.status === "active");
    const passthrough = records.filter((record) => record.status !== "active");
    const kept: MemoryRecord[] = [];
    const retired: MemoryRecord[] = []; // merged-away losers, kept recoverable (not destroyed)
    ...
    const indexKept = (index: number, vector: EmbeddingVector | undefined, type: string): void => {
      if (!vector) return;
      for (const key of bandKeys(vector)) {
        const full = `${type}|${key}`;
        const list = buckets.get(full);
        if (list) list.push(index);
        else buckets.set(full, [index]);
      }
    };
    ...
    for (const record of active) {
      ...
      processed += 1;
      if (processed % DEDUP_YIELD_EVERY === 0) await new Promise((resolve) => setImmediate(resolve));
      ...
      if (vec) {
        if (options.exhaustive) {
          for (let i = 0; i < kept.length; i += 1) {
            if (kept[i].type !== record.type) continue;
          ...
          for (const key of bandKeys(vec)) {
            ...
            for (let c = candidates.length - 1; c >= 0; c -= 1) {
              if (examined >= DEDUP_MAX_CANDIDATES) break;
            ...
            if (matchIndex !== -1 || examined >= DEDUP_MAX_CANDIDATES) break;
          }
        }
      }
      if (matchIndex === -1) {
      ...
      if (canonical.id === record.id) indexKept(matchIndex, vec, record.type);
    ...
    const resolveRemap = (id: string): string => {
      let cur = id;
    ...
    const fixed = [...passthrough, ...retired].map((record) =>
      record.supersededBy && remap.has(record.supersededBy)
  ...
  async readProcessingState(): Promise<ProcessingState> {
    const raw = await readFile(join(this.memoryDir, "brain", "processing-state.json"), "utf8").catch(() => "");
    if (!raw.trim()) return {};
    try {
      return JSON.parse(raw) as ProcessingState;
    } catch {
      return {};
    }
  }

  async writeProcessingState(state: ProcessingState): Promise<void> {
  ...
  private requireSession(sessionId: string): PeonSession {
    const session = this.sessions.get(sessionId);
    if (!session) throw new Error(`Unknown Peon session: ${sessionId}`);
    return session;
  }
  ...
  private async updateBrain(event: PeonEvent): Promise<void> {
  ...
  private async appendTimeline(event: PeonEvent): Promise<void> {
    await this.appendMarkdown("brain/timeline.md", `- ${event.createdAt} [${event.type}] ${event.content}\n`);
  }

  private async writeSessionSummary(session: PeonSession): Promise<void> {
  ...
  private async applyStructuredMemory(memory: ProcessedMemory, source: { reason?: string }, modelEntities?: Map<string, string[]>): Promise<ApplyMemoryStats> {
    const modelEntitiesFor = (content: string): string[] => modelEntities?.get(content.trim()) ?? [];
    ...
    for (const operation of memory.operations ?? []) {
      ...
      if (!target) continue; // unknown / hallucinated id → drop (degrade to add-only)
      if (target.status === "superseded") continue; // idempotency: already settled
      ...
      if (!replacement || typeof replacement.content !== "string" || !replacement.content.trim()) continue;
      const replacementId = stableMemoryId(replacement.type, replacement.content);
      if (replacementId === target.id) continue; // self-supersede → no-op
      target.status = "superseded";
    ...
    const records = Array.from(existingByKey.values()).sort((left, right) =>
      left.type === right.type ? left.content.localeCompare(right.content) : left.type.localeCompare(right.type)
    );
    await this.replaceMemoryRecords(records);
    return { superseded, obsoleted, added };
  }

  private async readMemoryRecords(): Promise<MemoryRecord[]> {
  ...
  private async readMemoryGraph(): Promise<MemoryGraph> {
    const raw = await readFile(join(this.memoryDir, "brain", "graph.json"), "utf8").catch(() => "");
    if (!raw.trim()) return emptyGraph();
    try {
  ...
  private async appendJsonl(relativePath: string, value: unknown): Promise<void> {
    await this.appendMarkdown(relativePath, `${JSON.stringify(value)}\n`);
  }

  private async appendMarkdown(relativePath: string, content: string): Promise<void> {
    await appendFile(join(this.memoryDir, relativePath), content, "utf8");
  }

  private async appendList(relativePath: string, items: string[]): Promise<void> {
    if (items.length === 0) return;
    await this.appendMarkdown(relativePath, items.map((item) => `- ${item}`).join("\n") + "\n");
  }

  private async readJsonl(relativePath: string): Promise<Array<Record<string, unknown>>> {
...
function normalizeContextBudget(maxChars: number | undefined): number {
  if (maxChars === undefined || !Number.isFinite(maxChars)) return 24000;
...
function contextTerms(query: string | undefined): string[] {
  if (!query) return [];
  return query
    .toLowerCase()
    .split(/[^a-z0-9_.-]+/)
...
function isUsefulContextLine(line: string): boolean {
  return ![
...
function processedMemoryToRecords(memory: ProcessedMemory): MemoryRecordInput[] {
  return [
    ...(memory.summary.trim() ? [{ type: "summary" as const, content: memory.summary }] : []),
    ...memory.decisions.map((content) => ({ type: "decision" as const, content })),
    ...memory.preferences.map((content) => ({ type: "preference" as const, content })),
    ...memory.openQuestions.map((content) => ({ type: "open_question" as const, content })),
    ...memory.artifacts.map((content) => ({ type: "artifact" as const, content })),
    ...memory.timeline.map((content) => ({ type: "timeline" as const, content })),
    ...(memory.memories ?? [])
 ...
 */
function formatContextRecords(ranked: RankedMemoryRecord[], maxChars: number, title: string): string {
  if (ranked.length === 0 || maxChars <= 0) return "";
  const header = `# ${title}\n`;
  let out = header;
 ...
 */
function formatEpisodes(ranked: RankedMemoryRecord[], maxChars: number): string {
  if (ranked.length === 0 || maxChars <= 0) return "";
...
function formatRankedMemoryRecords(ranked: RankedMemoryRecord[]): string {
...
function formatInjectionPreview(context: ProjectContext): string {
  return [
...
function mergeScore(
  left: MemoryRecord["score"],
  right: MemoryRecord["score"]
): MemoryRecord["score"] {
  return {
 ...
 */
export function memoryRecordId(type: MemoryType, content: string): string {
...
function inferEntities(content: string, extra: readonly string[] = []): string[] {
 ...
 */
function deriveProvenance(content: string, capturedAt: string): MemoryRecord["provenance"] {
  const url = content.match(/https?:\/\/[^\s)]+/);
  if (url) return { kind: "url", ref: url[0], capturedAt };
  ...
  if (file) {
    ...
    if (canonical) return { kind: "file", ref: canonical.key, capturedAt };
  }
  return { kind: "episodic", ref: capturedAt, capturedAt };
}

function buildMemoryGraph(projectName: string, records: MemoryRecord[]): MemoryGraph {
  const projectNode = { id: nodeId("project", projectName), type: "project", label: projectName };
  const nodes = new Map<string, MemoryGraph["nodes"][number]>([[projectNode.id, projectNode]]);
  const edges = new Map<string, MemoryGraph["edges"][number]>();
  ...
  const registryNamespace = new Map<string, string>();
  for (const record of records) {
    for (const raw of record.entities) {
      const c = canonicalizeEntity(raw);
      if (c) registryNamespace.set(canonical.get(raw) ?? c.key, c.namespace);
    }
  }
...
export function defaultMaxDeltaChars(): number {
