 ...
 */
export function currentAsOf(records: MemoryRecord[], at: string | number | Date): MemoryRecord[] {
  const t = ms(at);
  return records.filter((r) => {
    if (ms(r.createdAt) > t) return false; // didn't exist yet
    if (TERMINAL_STATUSES.has(r.status)) return ms(r.updatedAt) > t; // retired only after `at`
...
export interface ChangeEntry {
  ...
  record: MemoryRecord;    // the belief that changed (the OLD one for superseded/retired)
  replacementId?: string;  // successor id for `superseded`
}
