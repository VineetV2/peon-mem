import { diversifyByMMR, rankMemoryRecords, type RankedMemoryRecord, type RetrievalReason } from "./retrieval.js";
...
export function buildContextInjection(options: BuildContextInjectionOptions): ContextInjection {
  ...
  const omitted: OmittedInjectionMetadata[] = [];
  const relevant = [...projectResults, ...globalResults]
    .filter((candidate) => {
      if (includeInactive || candidate.item.record.status === "active") return true;
...
function compareCandidates(left: InjectionCandidate, right: InjectionCandidate): number {
...
function injectionScore(item: RankedMemoryRecord): number {
  if (item.record.status === "stale") return item.score - 4;
  if (item.record.status === "conflicted") return item.score - 1;
  return item.score;
}

function formatCandidate(item: RankedMemoryRecord): string {
...
function selectedMetadata(item: RankedMemoryRecord, chars: number): SelectedInjectionMetadata {
  return {
    id: item.record.id,
...
function omittedMetadata(item: RankedMemoryRecord, reason: OmittedInjectionReason): OmittedInjectionMetadata {
  return {
    id: item.record.id,
    scope: item.record.scope,
    type: item.record.type,
...
function whySelected(item: RankedMemoryRecord): string {
  if (item.explanation.trim().length > 0) return item.explanation.trim();
  const positiveReasons = item.reasons.filter((reason) => reason.score > 0);
  if (positiveReasons.length > 0) return positiveReasons.map(formatReason).join("; ");
  return `score ${item.score.toFixed(3)}`;
}

function formatReason(reason: RetrievalReason): string {
  return reason.label;
}
