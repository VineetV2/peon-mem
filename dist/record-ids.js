...
export function ensureUniqueRecordIds(records) {
    ...
    if (shared.length === 0)
        return { records: [...records], reassigned: 0 };
    ...
    for (const [id, indexes] of shared) {
        const ordered = [...indexes].sort((a, b) => statusRank(records[a]) - statusRank(records[b]) ||
        ...
        for (const index of ordered.slice(1)) {
            ...
            for (let n = 2; taken.has(candidate); n += 1)
                candidate = `${id}_${fnv1a(`${seed}\n${n}`)}`;
            taken.add(candidate);
            newIdByIndex.set(index, candidate);
            for (const member of copy.summaryOf ?? []) {
                if (keeperMembers.has(member))
                    continue;
                const moves = memberMoves.get(member) ?? new Map();
...
const STATUS_RANK = { active: 0, conflicted: 1, stale: 2, superseded: 3, archived: 4 };
